...
namespace BlueNESTest
{
	...
	public:
		TEST_METHOD_INITIALIZE(TestSetup)
		{
			...
			cart->mapper->SetPRGRom(prgRom.data(), prgRom.size());

			// Initialize 8 KB CHR RAM
			std::vector<uint8_t> chrRam(8 * 1024, 0);
			cart->mapper->SetCHRRom(chrRam.data(), chrRam.size());
		...
		TEST_METHOD(TestSUROMBanking)
		{
			...
			// Verify PRG mapping
			uint8_t prgData = bus->read(0x8000); // Read from $8000
			...
			// Verify PRG mapping
			prgData = bus->read(0x8000); // Read from $8000
