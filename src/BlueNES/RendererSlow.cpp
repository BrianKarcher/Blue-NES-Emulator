...
RendererSlow::RendererSlow() {
	m_scrollX = 0;
	m_scrollY = 0;
	m_backBuffer.fill(0xFF000000); // Initialize to opaque black
}

void RendererSlow::Initialize(NesPPU* ppu) {
	this->ppu = ppu;
	bus = &ppu->core->bus;
}

void RendererSlow::SetScrollX(uint8_t value) {
	m_scrollX = value;
}

void RendererSlow::SetScrollY(uint8_t value) {
	m_scrollY = value;
}
...
void RendererSlow::EvaluateSprites(int screenY, std::array<Sprite, 8>& newOam)
{
	for (int i = 0; i < 8; ++i) {
...
void RendererSlow::RenderScanline()
{
...
void RendererSlow::get_palette_index_from_attribute(uint8_t attributeByte, int tileRow, int tileCol, uint8_t& paletteIndex)
{
